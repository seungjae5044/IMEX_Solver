...
class UNet(nn.Module):
    ...
    def forward(self, x, time):
        ...
        input_image_size = x.shape[-2:]  # (height, width)  # assumes square image
        
        feats = []
        now_res = input_image_size
        for layer in self.downs:
            if isinstance(layer, ResnetBlocWithAttn):
                layer.with_attn = (now_res in self.attn_res)
                x = layer(x, t)
            else:
                x = layer(x)
        ...
        for layer in self.ups:
            if isinstance(layer, ResnetBlocWithAttn):
                ...
                if feat.shape[-1] != x.shape[-1] or feat.shape[-2] != x.shape[-2]:
                    feat = F.interpolate(feat, size=x.shape[-2:], mode='nearest')
                x = layer(torch.cat((x, feat), dim=1), t)
            else:
                x = layer(x)

        x = self.final_conv(x)
        if x.shape[-2:] != input_image_size:
            x = F.interpolate(x, size=input_image_size, mode='nearest')
        return x